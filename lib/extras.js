...
function extendRoutes (routes, options, resolve, { srcDir, rootDir }) {
  ...
  for (const route of routes) {
    ...
    if (objectAlias.length > 0) {
      additionalRoutes = additionalRoutes.concat(mapObjectAliases(route, objectAlias))
    }
    ...
    if (simpleAlias.length > 0) {
      if (options.routerNativeAlias) {
        ...
        additionalRoutes = additionalRoutes.concat(mapSimpleAliases(route, simpleAlias))
      }
    }
  ...
  const insertIndex = routes.findIndex(route => /[*]/.test(route.path)) || routes.length - 1
  routes.splice(insertIndex, 0, ...additionalRoutes)
}
...
function invalidateRoute (file) {
  const routes = localRoutes.filter(route => route.component.replace(/\\\\/g, '\\') === file)
